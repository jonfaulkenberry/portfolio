...
feature "As an owner, I want to edit any post" do
  ...
  scenario "owner edits another users post successfully" do
    @another_user = create(:author)
    @post = create(:post, author: @another_user)
    visit post_path(@post)
    click_on "Edit Post"
    title = Forgery(:lorem_ipsum).words(10, :random => true)
    body  = Forgery(:lorem_ipsum).words(250, :random => true)
...
feature "As an author, I want to edit my own posts" do
  background do
    sign_in_as_author
    @post = create(:post)
  end
  scenario "author edits own post successfully" do
    @post = create(:post, author: @user)
    visit post_path(@post)
    click_on "Edit Post"
    title = Forgery(:lorem_ipsum).words(10, :random => true)
  ...
  scenario "author cannot edit another users post" do
    @another_user = create(:author)
    @post = create(:post, author: @another_user)
    visit post_path(@post)
